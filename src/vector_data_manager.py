...
def load_document_store(document_store_dir=".data/document_store/", index_name="new_faiss_index.faiss", database_name="faiss_document_store.db", vector_dim=128):
    ...
    if not os.path.exists(index_path):
        document_store = FAISSDocumentStore(sql_url="sqlite:///"+document_store_dir+database_name,
                                            vector_dim=vector_dim, faiss_index_factory_str="Flat")
    else:
...
def prepare_documents(data_path):
    ...
    df["meta"] = df.apply(
        lambda x: {"name": x["Idea Title"], "id": x["Idea ID"]}, axis=1)
